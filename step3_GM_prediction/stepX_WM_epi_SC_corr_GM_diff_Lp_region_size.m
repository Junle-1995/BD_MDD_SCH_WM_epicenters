function stepX_WM_epi_SC_corr_GM_diff_Lp_region_size(GM_atlas,GM_roi_num,s_min,s_max)
%% corr the GM volume diff Z map with Lp of SC to the WM epicenter
path = pwd;
...
load('Results_epicenter_diff_SC_Lp_region_size.mat')
lp_results = results;
...
for i = 1:length(lp_results)
    cd(path_data)
    load(['unrelated_SC_' lp_results(i).type '_' GM_atlas '_JHU.mat'],'SC')
...
%% save
cd(path)
save(['Results_WM_epicenter_SC_' GM_atlas '_diff_Lp_region_size.mat'],'results')
