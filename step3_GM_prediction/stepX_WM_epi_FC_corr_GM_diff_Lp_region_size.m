function stepX_WM_epi_FC_corr_GM_diff_Lp_region_size(GM_atlas,GM_roi_num,s_min,s_max)
%% corr the GM volume diff Z map with Lp of FC to the WM epicenter
path = pwd;
...
load('Results_epicenter_diff_FC_Lp_region_size.mat')
lp_results = results;
...
cd ..
[Net, ~] = gretna_batch_lp_bin_all(FC, s_min, s_max, 0.02, 'pos', 's');
...
%% save
cd(path)
save(['Results_WM_epicenter_FC_' GM_atlas '_diff_Lp_region_size.mat'],'results')
