function stepX_WM_epi_SC_corr_GM_diff_region_size(GM_atlas,GM_roi_num)
%% corr the GM volume diff Z map with SC to the WM epicenter
path = pwd;
...
load('Results_epicenter_diff_SC_region_size.mat')
con_results = results;
...
for i = 1:length(con_results)
    cd(path_data)
    load(['unrelated_SC_' con_results(i).type '_' GM_atlas '_JHU.mat'],'SC')
    Net = SC;
...
%% save
cd(path)
save(['Results_WM_epicenter_SC_' GM_atlas '_diff_region_size.mat'],'results')
