%% find the epicenter of WM using Lp with HCP WM co-variance matrix in BD and SCH
path = pwd;
    ...
    for igroup = 1:length(Group)
        ...
        for iroi = 1:roi_num
            ...
            results_abs(end).rho_abs_rand(iroi,:) = ...
                partialcorr(surrogate_maps(:,roi_index)',abs(SC_abs(roi_index,iroi)),region_size(roi_index),'type','Spearman');
        end
...
%% save
cd(path)
save('Results_epicenter_diff_SC_Lp_region_size.mat','results','results_abs')