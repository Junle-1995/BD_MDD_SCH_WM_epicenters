%% find the epicenter of WM with HCP mean FC matrix in BD
path = pwd;
...
cd(path_data)
load('unrelated_FC_JHU.mat','FC')
FC_abs = mean(abs(FC),3);
FC = mean(FC,3);
...
for itype = 1:length(Type)
    for igroup = 1:length(Group)
        ...
        results_abs(end).Net = abs(FC_ori);
        ...
        for iroi = 1:roi_num
            ...
            results_abs(end).rho_abs(iroi,1) = ...
                partialcorr(results_abs(end).Z(roi_index),abs(FC_abs(roi_index,iroi)),region_size(roi_index),'type','Spearman');
            ...
            results_abs(end).rho_abs_rand(iroi,:) = ...
                partialcorr(surrogate_maps(:,roi_index)',abs(FC_abs(roi_index,iroi)),region_size(roi_index),'type','Spearman');
        end
...
%% save
cd(path)
save('Results_epicenter_diff_FC_region_size.mat','results','results_abs')