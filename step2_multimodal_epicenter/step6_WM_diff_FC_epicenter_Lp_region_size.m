%% find the epicenter of WM using Lp with HCP mean FC matrix in BD
path = pwd;
...
cd(path_data)
load('unrelated_FC_JHU.mat')
FC_ori = FC;
[FC, ~] = gretna_batch_lp_bin_all(FC_ori, 0.09, 0.3, 0.02, 'pos', 's');
FC_abs = gretna_batch_lp_bin_all(abs(FC_ori), 0.09, 0.3, 0.02, 'pos', 's');
    ...
    for igroup = 1:length(Group)
        ...
        for iroi = 1:roi_num
            ...
            results(end).rho(iroi,1) = ...
                partialcorr(results(end).Z(roi_index),FC(roi_index,iroi),region_size(roi_index),'type','Spearman');
            results_abs(end).rho_abs(iroi,1) = ...
                partialcorr(results_abs(end).Z(roi_index),FC_abs(roi_index,iroi),region_size(roi_index),'type','Spearman');

            results(end).rho_rand(iroi,:) = ...
                partialcorr(surrogate_maps(:,roi_index)',FC(roi_index,iroi),region_size(roi_index),'type','Spearman');
            results_abs(end).rho_abs_rand(iroi,:) = ...
                partialcorr(surrogate_maps(:,roi_index)',FC_abs(roi_index,iroi),region_size(roi_index),'type','Spearman');
        end
...
%% save
cd(path)
save('Results_epicenter_diff_FC_Lp_region_size.mat','results','results_abs')